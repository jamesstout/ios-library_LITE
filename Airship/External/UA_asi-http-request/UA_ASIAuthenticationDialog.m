...
#import "UA_ASIAuthenticationDialog.h"
...
@implementation UA_ASIAutorotatingViewController
...
@interface UA_ASIAuthenticationDialog ()
- (void)showTitle;
- (void)show;
...
@implementation UA_ASIAuthenticationDialog

#pragma mark init / dealloc

+ (void)initialize
{
	if (self == [UA_ASIAuthenticationDialog class]) {
		requestsNeedingAuthentication = [[NSMutableArray array] retain];
	}
}

+ (void)presentAuthenticationDialogForRequest:(UA_ASIHTTPRequest *)theRequest
{
	...
	if (!sharedDialog) {
		...
		if ([theRequest authenticationNeeded] == UA_ASIProxyAuthenticationNeeded) {
			[sharedDialog setType:UA_ASIProxyAuthenticationType];
		} else {
			[sharedDialog setType:UA_ASIStandardAuthenticationType];
		}
		[sharedDialog show];
	} else {
...
- (UIViewController *)presentingController
{
	if (!presentingController) {
		presentingController = [[UA_ASIAutorotatingViewController alloc] initWithNibName:nil bundle:nil];
...
- (void)showTitle
{
	...
	if (UIInterfaceOrientationIsPortrait([[UIDevice currentDevice] orientation])) {
		// Setup the title
		if ([self type] == UA_ASIProxyAuthenticationType) {
...
- (void)cancelAuthenticationFromDialog:(id)sender
{
	for (UA_ASIHTTPRequest *theRequest in [self requestsRequiringTheseCredentials]) {
...
- (NSArray *)requestsRequiringTheseCredentials
{
	...
	for (UA_ASIHTTPRequest *otherRequest in requestsNeedingAuthentication) {
...
- (void)presentNextDialog
{
	if ([requestsNeedingAuthentication count]) {
		UA_ASIHTTPRequest *nextRequest = [requestsNeedingAuthentication objectAtIndex:0];
...
- (void)loginWithCredentialsFromDialog:(id)sender
{
	for (UA_ASIHTTPRequest *theRequest in [self requestsRequiringTheseCredentials]) {
		...
		// Handle NTLM domains
		NSString *scheme = ([self type] == UA_ASIStandardAuthenticationType) ? [[self request] authenticationScheme] : [[self request] proxyAuthenticationScheme];
